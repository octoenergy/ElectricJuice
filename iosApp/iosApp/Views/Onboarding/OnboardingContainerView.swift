...
struct OnboardingContainerView: View {
    ...
    @State private var onboardingPages: [OnboardingPage] = [OnboardingPage(title: "Welcome to Electric Juice", subtitle: "Find, charge and pay in one place, with our ever-expanding network of charge points across the UK."),
                ...
                PageContainerView(pageCount: onboardingPages.count, currentIndex: $currentIndex) {
                    ForEach(0..<onboardingPages.count) { index in
                        OnboardingPageView(title: self.onboardingPages[index].title,
                            ...
                            imageName: self.onboardingImages[index],
                            pageIndex: index)
                    }
                }
                VStack {
